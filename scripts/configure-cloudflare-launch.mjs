...
function readKeychain(service) {
  try {
    return execFileSync("security", ["find-generic-password", "-s", service, "-a", "codex-env", "-w"], {
      encoding: "utf8",
...
const cfToken = process.env.CLOUDFLARE_API_TOKEN || readKeychain("CLOUDFLARE_API_TOKEN");
const cfGlobalKey = process.env.CLOUDFLARE_API_KEY || readKeychain("CLOUDFLARE_API_KEY");
const cfEmail = process.env.CLOUDFLARE_EMAIL || process.env.CLOUDFLARE_API_EMAIL || readKeychain("CLOUDFLARE_EMAIL");
const spaceshipKey = (process.env.SPACESHIP_API_KEY || readKeychain("SPACESHIP_API_KEY")).trim();
const spaceshipSecret = (process.env.SPACESHIP_API_SECRET || readKeychain("SPACESHIP_API_SECRET")).trim();
...
async function updateRegistrarNameservers(zone) {
  if (!spaceshipKey || !spaceshipSecret) return { action: "skipped", reason: "Spaceship credentials not configured" };
  ...
  const nameservers = Array.isArray(payload) ? payload : payload.hosts || [];
